# Matrix Continuwuity + LiveKit (HowTo)

Здесь описан процесс развертывания Matrix-сервера Continuwuity с поддержкой видеозвонков через LiveKit на Synology NAS (DSM 7) с использованием Container Manager (Docker).
...
```bash
curl -sLO https://raw.githubusercontent.com/arabezar/howto-synology-matrix-continuwuity/main/install.sh && chmod +x install.sh && ./install.sh
```
Отвечайте на вопросы, следите за исполнением скрипта. После завершения скрипта перейдите в `Container Manager`, создайте и запустите проект: `Проект` > `Создать` > `Название проекта` (например, `matrix-continuwuity`) + `Путь` > `Задать` (например, `/docker/matrix-continuwuity`) > `Использовать существующий файл docker-compose.yml для создания проекта` > `Далее`…
<details>
...
✅ Установка Matrix Continuwuity завершена, создайте и запустите проект в Container Manager
```
</details>

### Создание пользователей
Первого пользователя (администратора) необходимо создать в UI [Element Web](https://app.element.io/): указываем основной домен, создаём пользователя, останавливаем контейнеры, меняем в `continuwuity.toml` параметр `allow_registration = false` и запускаем контейнеры.

Последующих пользователей можно создавать в комнате администратора командой `!admin users create-user <username> <password>`
...
### Переустановка
> [!TIP]
> Если в момент установки что-то не устроило, всегда можно прервать процесс развёртывания и начать его снова (`./install.sh`). Любые конфигурационные файлы могут быть удалены, инсталлятор загрузит их и настроит повторно. С другой стороны, если какие-то файлы не удалять, то загружаться и настраиваться повторно они не будут.

> [!IMPORTANT]
> Папка `db` хранит всю базу сообщений и пользователей вашего сервера, если удалить папку, вы потеряете и всех пользователей, и все сообщения, соответственно, придётся заново регистрировать всех пользователей, включая администратора.

> [!CAUTION]
> Файл `.env` хранит все настройки, введённые пользователем, и автоматически подхватывается инсталлятором. Также этот файл играет важную роль для автонастройки конфигурационных файлов `compose.yaml` и `proxy.conf.template`, однако, если в файле вручную изменить какие-то переменные, они не будут автоматически обновлены в конфигурационных файлах `continuwuity.toml` и `livekit.yaml`. Самым простым и правильным способом изменения переменных в `.env` файле является удаление файлов `continuwuity.toml` и `livekit.yaml` и повторный запуск инсталлятора `./install.sh`, где вы можете переопределить переменные и пересоздать проект `Container Manager`.

### Удаление
Если по каким-то причинам требуется удалить развёрнутый проект, следует для проекта в `Container Manager` последовательно вызвать два пункта меню `Действия`: `Очистить` (будут остановлены, если запущены, и удалены контейнеры) и `Удалить` (будет удалён проект). Все данные и настройки в папке `matrix-continuwuity` останутся и удалены не будут, что позволит в любой момент пересоздать проект, указав в качестве проекта папку `matrix-continuwuity`.
...
✔ Server version: continuwuity (0.5.0 (f4ccb81))
...
```
> Не обращайте внимания на крестик внизу: при включении MSC4140 в `Dendrite`, последний начинает безбожно глючить, - видео рвётся сразу после подключения.
</details>
