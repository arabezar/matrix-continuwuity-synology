...
Создайте в Container Manager проект matrix-continuwuity, задайте путь /docker/matrix-continuwuity, запустите проект и продолжайте здесь... задайте имя пользователя-администратора [admin] (Enter - подтвердить):
...
```bash
docker exec -it matrix-continuwuity /usr/bin/create-account -config /etc/dendrite/dendrite.yaml -username <user_name>
```
